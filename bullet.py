...
class Bullet():
    ...
    def check_impact(self, plataform_list, enemy_list, player):
        ...
        # Si las balas colisionan con los enemigos
        if self.owner == player:  # Check if the owner is the player
            collided_enemy = None
            ...
            if collided_enemy:
                enemy_list.remove(collided_enemy)  # Remove the collided enemy from the list
                self.is_active = False

        # Si las balas colisionan con la plataforma
    ...
    def remove(self):
        # Realizar acciones para eliminar o desactivar el objeto Bullet
