...
class FormGameLevel1(Form):
    def __init__(self,name,master_surface,x,y,w,h,color_background, imagen_background, color_border,active,config_json):
        ...
        self.text_lvl = Widget(master=self,x=5,y=25,w=200,h=50,color_background=None,color_border=None,image_background="images\gui\set_gui_01\Data_Border\Buttons\Button_XL_02.png",text="NIVEL 1",font='opensans.ttf',font_size=40,font_color=C_WHITE)
        ...
        # --- GAME ELEMENTS ---
        self.static_background = Background(x=0,y=0,width=w,height=h,path="images\\nevel_one.png")
    ...
    def generate_enemies_distance(self):
        data_enemies = self.levels[0]["enemies_distance"]
        for enemy in data_enemies:
    ...
    def generate_fruit(self):
        data_fruit = self.levels[0]["fruit"]
    ...
    def generate_trampas(self):
        data_trampa = self.levels[0]["trampa"]
    ...
    def check_collision(self):
        for bullet in self.player_1.bullet_list:
            for enemy in self.enemies_list_distance:
                if bullet.is_active and bullet.rect.colliderect(enemy.rect):
                    bullet.is_active = False  # Desactivar la bala después de la colisión con un enemy_distance
                    enemy.remove()  # Eliminar el enemy_distance colisionado
