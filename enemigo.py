...
class Enemy():
    def __init__(self, x, y, speed_walk, gravity, frame_rate_ms, move_rate_ms, interval_time_jump=100, p_scale=1):
    ...
    def do_animation(self, delta_ms):
        ...
        if self.tiempo_transcurrido_animation >= self.frame_rate_ms:
            self.tiempo_transcurrido_animation = 0
            self.frame += 1  # Incrementar el valor de self.frame

            if self.frame >= len(self.animation):
                self.frame = 0  # Reiniciar self.frame si excede el rango de la lista

        self.image = self.animation[self.frame]  # Asignar self.image utilizando el valor corregido de self.frame
    ...
    def draw(self, screen):
        ...
        if self.frame < len(self.animation):
            ...
            # Si self.frame está fuera del rango, establecerlo al último índice válido
